...
void NotPad::changeEvent(QEvent* event)
{
    switch(event->type())
    {
    case QEvent::ThemeChange: /// NOTE Seems to be undocumented event type
        {
            qDebug() << "ThemeChange";
            updateStyle();
            event->accept();
